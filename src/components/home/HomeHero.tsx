import heroImage from "@/assets/home/hero-home.jpg";
import { Button } from "@/components/ui/button";

const HomeHero = () => {
  return (
    <section className="relative bg-secondary overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-8 items-center min-h-[600px] py-12">
          <div className="order-2 md:order-1">
            <span className="inline-block bg-primary/10 text-primary font-heading font-semibold text-xs uppercase tracking-wider px-4 py-1.5 rounded-full mb-6">
              Uma casa sem obstáculos
            </span>
            <h1 className="font-heading font-extrabold text-3xl md:text-4xl lg:text-5xl text-foreground leading-tight mb-5">
              Soluções para viver em sua casa com{" "}
              <span className="text-primary">liberdade e independência</span>
            </h1>
            <p className="font-body text-lg text-muted-foreground leading-relaxed mb-8 max-w-lg">
              No Vida Acessível damos-lhe a conhecer as soluções de
              acessibilidade que tornam o dia a dia em casa mais confortável,
              seguro e simples — com informação clara, comparação de
              equipamentos e orientação especializada.
            </p>
            <div className="flex flex-wrap gap-3">
              <Button variant="cta" size="lg">
                Orçamentos
              </Button>
              <Button variant="outline" size="lg" className="rounded-full">
                Catálogo
              </Button>
            </div>
          </div>

          <div className="order-1 md:order-2 relative">
            <div className="rounded-2xl overflow-hidden shadow-2xl shadow-primary/10">
              <img
                src={heroImage}
                alt="Cadeira elevatória instalada numa escada residencial moderna"
                className="w-full h-[350px] md:h-[450px] object-cover"
                loading="eager"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default HomeHero;
